...
class Kart(models.Model):
    ...
    track = models.ForeignKey(Track, on_delete=models.CASCADE, related_name='karts')
...
class Driver(models.Model):
    ...
    track = models.ForeignKey(Track, on_delete=models.CASCADE, related_name='drivers')
...
class Heat(models.Model):
    """Заезд"""
    external_id = models.IntegerField(unique=True)  # ID с сайта
    track = models.ForeignKey(Track, on_delete=models.CASCADE, related_name='heats')
    name = models.CharField(max_length=200, blank=True)
    scheduled_at = models.DateTimeField()  # Дата и время
    laps_count = models.PositiveSmallIntegerField(default=0)  # Общее количество кругов
...
class HeatParticipation(models.Model):
    ...
    kart = models.ForeignKey(Kart, on_delete=models.CASCADE, related_name='participations')

    # Результаты заезда
    position = models.PositiveSmallIntegerField()  # Итоговая позиция
    laps_completed = models.PositiveSmallIntegerField(default=0)  # Пройдено кругов
